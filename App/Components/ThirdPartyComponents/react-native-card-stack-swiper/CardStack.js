...
class CardStack extends Component {
  ...
  initDeck() {
    ...
    //NOTE: トランプの裏側を0番目にしている。
    const cardC = cards[0] || null;
    this.setState({
      cards,
      cardA,
      cardB,
  ...
  render() {
    ...
    const { drag, dragDistance, cardA, cardB, topCard, sindex, cardC } = this.state;
    ...
    return (
      <View {...this._panResponder.panHandlers} style={[{ position: 'relative' }, this.props.style]}>
        ...
        >
          {topCard !== 'cardB' && this.state.isSwiping ? cardC : cardB}
        </Animated.View>
        <Animated.View
        ...
        >
          {topCard !== 'cardA' && this.state.isSwiping ? cardC : cardA}
        </Animated.View>
      </View>
    );
  }
